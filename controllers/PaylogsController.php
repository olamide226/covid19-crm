 ...
 */
class PaylogsController extends Controller
{
    const ecrm_cat_id = 4; //loan reconciliation = 1 on ecrm_category table
    public $username;
    /**
     * @inheritdoc
     */
    ...
    {
        $searchModel = new PaylogsAdminSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
    ...
    {
        $searchModel = new PaylogsSlaSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
     ...
     * @param integer $id
     * @return Paylogs the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Paylogs::findOne($id)) !== null) {
            return $model;
        }
