...
<div class="complaint-form">
    ...
    <div class="row">
      <div class="col-md-4">
        ...
        <?= $form->field($model, 'state_id')->widget(Select2::classname(), [
          'data' => ArrayHelper::map(State::find()
          ->all(), 'id', 'state_name'),
          'language' => 'en',
      ...
      <div class="col-md-4">
        ...
        <p id="validate-lga" style="color:#A94442;"></p>
      </div>
      <div class="col-md-4">
    ...
    <div class="row">
      <div class="col-md-6">
        <?= $form->field($model, 'sub_category')->dropdownList(ArrayHelper::map(SubCategory::find()->all(), 'id', 'sub_category'),
...
//phone no. validation
  $('body').on('keyup', '#complaint-phone_no', function () {
    ...
    let phone = $('#complaint-phone_no').val(),
        regExp = /^[(]{0,1}[0-9]{11}[)]{0,1}$/,
        phoneValid = regExp.test(phone);
...
function popcomment(){
 ...
 if (phone !=="") {
  $.get("index.php?r=site/stats&comment=" + phone,
   function( data ) {
    $("select#complaint-comment_id").html(data);
  });
 }
...
//populate complaints with selected category
$('#complaint-sub_category').change(function () {

  $.get("index.php?r=site/stats&catId="  + parseInt($(this).val()),
    function( data ) {
      $("select#complaint-comment_id").html(data);
    });
});
...
//validate lga to be required
$('form').submit(function () {
  let lga = $('select#complaint-lga_id').val();

  if (lga == null || lga == 'Select LGA...'){
    $('#validate-lga').html("LGA cannot be blank.");
    return false;
  }else{
    $('#validate-lga').html("");
    return true;
