         ...
         ?>
               <tr>
                  <td><?php if ($row['ticket_status'] == 'open' || ($row['ticket_status'] == 'resolved' && time() - $row['edate'] <= 86400 )) {
                     echo Html::a(nl2br("Update\nTicket"), ['conversations/sla-update', 'id' => $row['id']], ['class' => 'btn btn-warning btn-xs','target'=>'_blank' ]) ;
                     }else {
                        echo Html::a(nl2br("View\nTicket"), ['conversations/viewonly', 'id' => $row['id']], ['class' => 'btn btn-warning btn-xs','target'=>'_blank' ]) ;
                        // echo '#';
                     } ?></td>
