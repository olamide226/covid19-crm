...
$agents = $db->createCommand("SELECT * FROM user WHERE id !='{$current_user}' and status = 1 order by login_status")
             ->queryAll();
...
<head>
    ...
    <?php $this->head() ?>
    <link href="<?= Yii::getAlias('@web')?>/images/hgsf2.jpg" rel="shortcut icon" type="image/x-icon">
</head>
<body>
<?php $this->beginBody() ?>
<?php include 'style.php'; ?>
<div class="container-scroller" style=" background-color:#B9C151;">
  <!-- partial:partials/_navbar.html background-image: linear-gradient(90deg, red, yellow);-->
  <nav class="navbar  col-lg-12 col-12 p-0 fixed-top d-flex flex-row" style="background-color:#5B832D; ">
    <div class=" text-center navbar-brand-wrapper" style="background-color:; ">
      <a class="navbar-brand brand-logo" href="<?= Url::home(true) ?>" style="color:#fff; ">
        <?= Html::img('@web/images/hgsf2.jpg', ['alt'=>'COVID - 19','style'=>'height:70px; width:100px; margin-left:35px;']) ?>
      </a>
      <a class="navbar-brand brand-logo-mini" href="<?= Url::home(true) ?>" style="color:#B8C050; background-color:#5B832D; font-size:15px;">COVID 19</a>
    </div>
    <div class="navbar-menu-wrapper d-flex align-items-center" style="background-image: linear-gradient(0deg, #5B832D,#ACBE50);">
      <button class="navbar-toggler navbar-toggler d-none d-lg-block navbar-dark align-self-center mr-3" type="button" data-toggle="minimize">
        <span class="navbar-toggler-icon" style="color:#fff; "></span>
      </button>

      <ul class="navbar-nav ml-lg-auto d-flex align-items-center flex-row" style="height:50px">
        <?php if(!Yii::$app->user->isGuest && (User::hasRole('Admin') || User::hasRole('supervisor') || User::hasRole('boiBackendTeam'))): ?>
          <li class="nav-item">
            <!-- notification bell -->
        ...
        <li class="nav-item">
          <!--?= Html::img('@web/images/grl_wrk.gif', ['alt'=>Yii::$app->user->identity->last_name,'style'=>'height:50px;']) ?-->
        </li>

        <li class="nav-item user-icon">
          <?php if(!Yii::$app->user->isGuest){ ?>
            <?= 'Welcome'.' '.Yii::$app->user->identity->first_name; ?>
          <?php } else {
            echo 'Guest User';
          } ?>
  ...
  <div class="container-fluid">
    ...
    <div class="row row-offcanvas row-offcanvas-right" style="background-color:#B5C15B;">
      ...
      <?php //if (Yii::$app->user->id > 1 && User::hasRole('boiBackendTeam')): ?>
      <?php //include 'boi-backend.php'; ?>
      <?php //endif; ?>
  ...
  <footer class="footer">
    <div class="container">
      <p class="pull-left">&copy; EBIS <?= date('Y') ?></p>
        <span>
          <center><a href="<?php echo Yii::$app->request->baseUrl; ?>/../cms" target="_blank" ></a></center>
        </span>
    </div>
  </footer>
  <!-- widget starts here -->
      ...
      <div class="row form-container">
        <div class="col-xs-12 col-sm-5" style="background-color:#2F4F4F; padding-right:0">
          ...
          <ul class="nav nav-tabs">
              ...
              <li class="active"><a data-toggle="tab" href="#recentChat">Recent Chat</a></li>
          </ul>
          ...
          <div class="" id="list" >
            <div class="list-group scrollbar scrollbar-primary"  style="height:350px;background-color:#2F4F4F; overflow: auto;">
              ...
              <div class="tab-content">
                ...
                <!-- nav-tab content: recentchat list-->
                <div id="recentChat" class="tab-pane fade in active recent-chat"></div>
              </div>
            </div>
          </div>
        </div>
...
$(document).ready(function(){
  ...
  // internal chat begins here
    ...
    //recentchat
    function updateRecentChatList(){
      $.get( "index.php?r=site/recentchat",
      function( data ) {
          $(".recent-chat").html(data);
      });
    }
