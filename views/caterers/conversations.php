...
<div class="panel panel-success">
  <div class="panel-heading"><h5> Log a new conversation.</h5></div>
    ...
    // Yii::$app->db opens a connection to the db. refer to pg 250 of yii2 guide pdf
            ...
            ->execute();
          $row = Yii::$app->db
            ->createCommand("SELECT CONCAT('3',MAX(CONCAT(DATE_FORMAT(create_date,'%Y%m%d'),LPAD(sequence,4,'0')))) AS gen from generator;")
            ->queryOne(); ?>
    ...
    <?= $form->field($calllogs, 'category_id')->hiddenInput(['value' => 3])->label(false) ?>
    ...
    <?= $form->field($calllogs, 'details')->textarea(['rows' => 2]) ?>

    <?= $form->field($calllogs, 'other_comment')->textarea(['rows' => 2]) ?>

    <?= $form->field($calllogs, 'cc_agent_response')->textarea(['rows' => 2]) ?>

    <?= $form->field($calllogs, 'phone_no')->hiddenInput(['value'=> $model->phone_number,
                                                    'readonly'=>false])->label(false) ?>
    ...
    <?= $form->field($calllogs, 'member_id')->hiddenInput(['value' => $model->member_id ? $model->member_id : $model->phone_number,
                                                        'readonly' => true])->label(false) ?>
    <?= $form->field($calllogs, 'account_no')->hiddenInput(['value'=> $model->account_no,'maxlength'=>true])->label(false) ?>
