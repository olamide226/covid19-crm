...
$conn = \Yii::$app->getDb();
$sql = sprintf("SELECT a.id, unix_timestamp(a.updated_date) edate, a.amount_paid, a.entry_date,a.ticket_number,a.ticket_status, concat(b.first_name, ' ', b.last_name) usernam, c.comments, a.other_comment
         ...
         ?>
               <tr>
                  <td><?php if ($row['ticket_status'] == 'open' || ($row['ticket_status'] == 'resolved' && time() - $row['edate'] <= 86400 )) {
                     echo Html::a(nl2br("Update\nTicket"), ['existpaylogs/sla-update', 'id' => $row['id']], ['class' => 'btn btn-warning btn-xs','target'=>'_blank' ]) ;
                     }else {
                        echo Html::a(nl2br("View\nTicket"), ['existpaylogs/viewonly', 'id' => $row['id']], ['class' => 'btn btn-warning btn-xs','target'=>'_blank' ]) ;
                        // echo '#';
                     } ?></td>
