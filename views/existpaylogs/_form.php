...
<div class="panel panel-success">
  <div class="panel-heading"><h5> Log a new conversation.</h5></div>
    ...
    <?= $form->field($model, 'category_id')->hiddenInput(['value' => 3])->label(false) ?>
    ...
    <?= $form->field($model, 'member_id')->hiddenInput(['readonly' => true])->label(false) ?>
...
$('#existpaylogs-fraud_status').change(function () {

  if ($('#existpaylogs-fraud_status').val() == '1') {
  $('#fraud-field').show();
} else {
  $('#fraud-field').hide();
        }
  ...
  $('#existpaylogs-entry_source_id').change(function () {
    if ($('#existpaylogs-entry_source_id').val() == '1') {
    $('#cs').show();
  } else {
    $('#cs').hide();
  }
